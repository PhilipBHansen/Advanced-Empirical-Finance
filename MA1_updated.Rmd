...
mom_portfolios <- crsp_monthly_mom %>%
  ...
  mutate(
    ...
    portfolio = case_when(
      mom <= breakpoint_1 ~ 10,
      mom > breakpoint_1 & mom <= breakpoint_2 ~ 20,
      mom > breakpoint_2 & mom <= breakpoint_3 ~ 30,
      mom > breakpoint_3 & mom <= breakpoint_4 ~ 40,
      mom > breakpoint_4 & mom <= breakpoint_5 ~ 50,
      mom > breakpoint_5 & mom <= breakpoint_6 ~ 60,
      mom > breakpoint_6 & mom <= breakpoint_7 ~ 70,
      mom > breakpoint_7 & mom <= breakpoint_8 ~ 80,
      mom > breakpoint_8 & mom <= breakpoint_9 ~ 90,
      mom > breakpoint_9 ~ 100
    )
  )
...
# Adds the Fama-French market excess return
mom_portfolios <- mom_portfolios %>%
  left_join(ff_monthly, by = "month")

# Finding CAPM alpha by regressing the data onto the market excess return
...
# Printing the results (using tmp to adjust data for the table output)
tmp <- mom_portfolios_summary
tmp$portfolio <- as.character(mom_portfolios_summary$portfolio)
tmp$alpha <- round(tmp$alpha,4)
tmp$beta <- round(tmp$beta,4)
tmp$ret_excess <- round(tmp$ret_excess,4)
t(tmp) %>%
knitr::kable(digits = 4) #creates a table to view the data.

# Plot for CAPM alpha
while (!is.null(dev.list()))  dev.off() # Nulstiller device settings (just in case, as it may cause problems)
