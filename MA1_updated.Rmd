...
row.names(long_short_summary) <- c("CAPM alpha", "Market beta")

# Using the linear regression values to depict as a table
long_short_summary %>%
  knitr::kable(booktabs = T, digits = 4, caption = "CAPM alpha, beta and excess return for the long-short portfolio based on value-weighted momentum-sorted portfolios") %>%
  kable_paper("hover", full_width = T)
...
The momentum strategy seems to... And the average return if....
...
# Creating the 3, 6, and 12 months ahead returns by moving back the respective month by 2, 5 or 11 months (since ret_excess is already 1-month ahead)
excess_return_3_ahead <-  crsp_monthly_mom %>% mutate(month = month %m+% months(-2)) %>% select(permno, month, ret_excess_3 = ret_excess) %>% drop_na()
excess_return_6_ahead <-  crsp_monthly_mom %>% mutate(month = month %m+% months(-5)) %>% select(permno, month, ret_excess_6 = ret_excess) %>% drop_na()
excess_return_12_ahead <-  crsp_monthly_mom %>% mutate(month = month %m+% months(-11)) %>% select(permno, month, ret_excess_12 = ret_excess) %>% drop_na()
...
# Finding the CAPM alpha by regressing the data onto the market excess return
mom_portfolios_k_ahead %>%
  group_by(portfolio) %>%
  summarise(
    alpha_1 = as.numeric(lm(ret_excess ~ mkt_excess)$coefficients[1]),
    alpha_3 = as.numeric(lm(ret_excess_3 ~ mkt_excess)$coefficients[1]),
    alpha_6 = as.numeric(lm(ret_excess_6 ~ mkt_excess)$coefficients[1]),
    alpha_12 = as.numeric(lm(ret_excess_12 ~ mkt_excess)$coefficients[1])
  ) %>%
  knitr::kable(booktabs = T, digits = 4, col.names = c("Portfolio number", "CAPM alpha", "Market beta", "Average value-weighted excess return"), caption = "CAPM alpha, beta and excess return for the value-weighted momentum-sorted portfolios") %>%
  kable_paper("hover", full_width = T)
