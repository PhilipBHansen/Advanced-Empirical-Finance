...
By using the sic2 variable to divide each stock into different industry classifications, we can in the following plot see how many stocks are included in each industry:
 ...
 characteristic_mom1m: 1-month momentum  -characteristic_mvel1: Logged market equity, or stock size   - characteristic_chmom: change in 6-monthmomentum  - characteristic_maxret: Maximum daily returns, from retruns during calendar month t-1  -characteristic_indmom: Industry momentum•macro_bm: The book-to-market ratio generally did well from 1940–1975, after which it did ratherpoorly•macro_ntis: Net Equity Expansion, poor performance in 1973–1974, zero performance otherwise.•macro_tbl: T-bill,the short-term rate had consistently good performance until around 1973, afterwhich it had consistently bad performance.•macro_tms: Term-Spread, the term-spread has mostly zero performance. In Panel A, it had a tinypositive drift until around 2000, in Panel C, it collapsed in the late 1970s.Our dataset also contains the following variables:•permno: The security identifier•month: The month data relevents to the the stock•ret_excess: The excess return•mktcap_lag: The market capitalization lagged in one month•sic2: The Standard Industrial Classification codes to identify each industry
...
reduced_scm %>% 
  ...
  filter(n_permno >= 75) %>% 
  ggplot(aes(x = reorder(sic2, n_permno), y = n_permno)) +
  geom_bar(stat = "identity") +
  labs(
    x = "Industry classifications (sic2)",
    y = "Number of firms (permno)",
    title = "Number of firms per industry classification",
    subtitle = "Top industry classifications"
  ) +
  coord_flip()
...
OBS: We are lost on how to answer the second half of exercise 2 (the part about using Asset Pricing Theory and what would be $z_{i,t}$ and the g-function. If you have any idea, please let us know, thanks.

- __What would be $z_{i,t}$?__ 
  - Macroeconomic factors.
- __What would be the functional form of $g(\cdot)$?__
  - To capture systematic risk.
...
scm_validation <- reduced_scm %>% filter(month >= "2014-07-01") %>% filter(month <= "2017-02-01")
...
# To test for 20% in test subset and validation subset (uncomment for testing)
# nrow(scm_test) / nrow(reduced_scm) * 100
# nrow(scm_validation) / (nrow(scm_validation) + nrow(scm_train)) * 100
