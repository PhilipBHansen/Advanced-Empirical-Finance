...
periods<-13-window_length #Number of periods
...
for(i in 1:3){ #Begin for loop for monthly portfolio estimation
  ...
  if(i>1){ 
    ...
    # HEA_return<-as.numeric(HEA_return_tmp)-HEA_TC[i]
    # NSS_return<-as.numeric(NSS_return_tmp)-NSS_TC[i]
    }
...
NP_turnover
(crsp_monthly_reduced %>% filter(month==months[15]) %>% select(ret_excess))$ret_excess %>% mean()
