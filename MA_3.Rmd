...
Where we have defined and used that $\mu^{*}=\mu + 2\lambda \Sigma \omega_{t^+}$ and $\Sigma^{*}=\left(\gamma + 2\lambda\right)\Sigma$. We can now use this to find a closed-form solution. To maximize a function is the same as minimizing the negative funtion:
\begin{align}
...
In order to find a closed-form solution, we define the Lagrange, differentiate it with respect to $\omega$ and $\lambda$ and then solve for $\omega$ (using that $\Sigma^{*}$ is symmetric):
\begin{equation*}
...
\end{align}
\begin{align}
    \frac{\partial\mathcal{L}\left(\mathbf{\omega}, \lambda\right)}{\partial\omega} = \iota^{\prime}\omega - 1 = 0 \quad \Leftrightarrow \quad \iota^{\prime}\omega = 1
\label{eq:5}
\end{align}
Equating \eqref{eq:4} and \eqref{eq:5} and soolve for $\lambda$:
\begin{align}
...
for(i in 1:(periods)){ #Begin for loop for monthly portfolio estimation
  ...
  r<-((crsp_monthly_reduced %>% filter(month == months[i+13]))$ret_excess)
...
#Calculate portfolio weights
 ...
 # NP_turnover[i]<- sum(abs(as.vector(NP)-as.vector(NP_return_tmp)))
#  HEA_turnover[i]<- sum(abs(as.vector(HEA)-as.vector(HEA_return_tmp)))
 # NSS_turnover[i]<- sum(abs(as.vector(NSS)-as.vector(NSS_return_tmp)))
 #   
#  NP_TC[i]<- lambda %*%(NP - NP_return_tmp)%*% sigma_hat %*% (NP - NP_return_tmp)
#  HEA_TC[i]<- lambda %*%(NP - NP_return_tmp)%*% sigma_hat %*% (NP - NP_return_tmp)
#  NSS_TC[i]<- lambda %*%(NP - NP_return_tmp)%*% sigma_hat %*% (NP - NP_return_tmp)
