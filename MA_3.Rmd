...
lambda_vector <- 20 * qexp((1:99)/100) 
...
```
The shape of the curve is what we would expect in this setup. With a transaction cost of 0 the optimal portfolio is perfectly rebalanced to match the efficient portfolio. As transaction cost increases the distance converges towards the distance between the naive portfolio and the efficient portfolio. This makes perfect sense, when transaction cost gets too high the investor simply sticks with the naive portfolio as it becomes impossible to generate a extra excess return large enough to compensate for the cost of moving away from the naive portfolio.
