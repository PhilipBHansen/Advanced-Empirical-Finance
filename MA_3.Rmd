...
1. The naive portfolio which rebalances daily to $\omega=\frac{1}{N}\iota$.
...
3. The mean-variance efficient portfolio with a no short-selling constraint in the spirit of Jagannathan and Ma (2003). We will further on call this portfolio "Mean-variance with TC". These portfolio weights are calculated by the *quadprog::solve.QP* function in R.

Before starting the rolling window out-of-sample estimation we start by defining the original portfolio as the naive portfolio along with defining a list of matrices where we can save the performance result for each strategy for each out-of-sample period. We now begin the rolling window out-of-sample estimation where we for each of the 459 periods start by filtering the data to only include the 240 months starting from the specified period (which continuously moves one month ahead). Then we also select the out-of-sample returns from the next month which wasn't included in the aforementioned data selection. From the data set consisting of 240 monthly observation we now calculate the standard $\hat{\mu}$ and the covariance matrix with shrinkage $\hat{\Sigma}$. These estimates are used to find the new optimal portfolio weights for  
