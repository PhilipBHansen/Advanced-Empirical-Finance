...
**48**: Part 1: exercise **MISSING** and Part 2: problem 3  
**49**: Part 1: exercise **MISSING** and Part 2: problem 2  
...
# Finding the portfolio turnover
months <- unique(mom_portfolios$month)
w_prev <- mom_portfolios %>% 
...
for(i in (2:length(months))){
  ...
  # Testing
  prev_length <- nrow(w_prev)
  new_length <- nrow(w_new)
  total_length <- nrow(tmp)
...
# Creating the returns for the long-short portfolio
better_strategy_raw <- mom_portfolios_raw %>%
...
# Creating monthly portfolio value-weighted excess returns
better_strategy <- better_strategy_raw %>%
...
# Creating the returns for the long-short portfolio
better_long_short_portfolio <- better_strategy %>%
...
# Making a linear regression of the portfolio return om the market return in order to get the CAPM values (and then renaming the rows)
better_long_short_summary <- summary(lm(long_short ~ mkt_excess, data=better_long_short_portfolio))$coefficients
tmp <- t.test(better_long_short_portfolio$long_short) # Creating a t.test for the return estimate
better_long_short_summary <- rbind(better_long_short_summary, c(tmp$estimate, tmp$stderr, tmp$statistic, tmp$p.value))
row.names(better_long_short_summary) <- c("CAPM alpha", "Market beta", "Portfolio return")

# Using the linear regression values to depict as a table
better_long_short_summary %>% knitr::kable(booktabs = T, digits = 4, caption = "CAPM alpha, beta and excess return for the long-short portfolio based on value-weighted momentum-sorted portfolios") %>%
                       kable_paper("hover", full_width = T) %>% 
                       kable_styling(latex_options = "HOLD_position")

# Finding the average return of the long-short portfolio
better_long_short_portfolio_mu <- mean(better_long_short_portfolio$long_short)

# Finding the portfolio turnover
months <- unique(better_strategy_raw$month)
w_prev <- better_strategy_raw %>% 
  filter(month == months[1]) %>% 
  ungroup() %>% 
  filter(updated_portfolio %in% c(1, 10)) %>% 
  mutate(weight = mktcap_lag / sum(mktcap_lag)) %>%
...
for(i in (2:length(months))){
  ...
  better_turnover <- better_turnover + sum(abs(tmp$weight.x - tmp$weight.y))
  
  # Testing
  prev_length_v2 <- nrow(w_prev)
  new_length_v2 <- nrow(w_new)
  total_length_v2 <- nrow(tmp)
  
  w_prev <- w_new
}

better_turnover <- better_turnover / length(months)
...
N<-US_Stocks$permno %>% unique() %>% length
theta<-rep(1,3)
Sub_US_Stocks<-US_Stocks %>% filter(month<"2016-01-01")


compute_parametric_portfolio_weights <- function(theta,
                                      data) {
  data %>%
    group_by(month) %>%
    mutate(
...
evaluate_portfolio <- function(weights_parametric_portfolio){
 ...
 tmp
  # return(-utility_function(tmp$Rp))
}


compute_objective_function <- function(theta,
                                       data) {
  processed_data <- compute_parametric_portfolio_weights(
    theta,
    data
  )
...
view(compute_parametric_portfolio_weights(theta, Sub_US_Stocks))
max((compute_parametric_portfolio_weights(theta, Sub_US_Stocks))$weight_tilt/0.125)
min((compute_parametric_portfolio_weights(theta, Sub_US_Stocks))$weight_tilt/0.125)

compute_objective_function(c(2415384027600149,  1259332216136819, -3175254693880788),Sub_US_Stocks)


tmp<-compute_parametric_portfolio_weights(theta,US_Stocks) 
tmp

sum(tmp$weight_tilt*(1+tmp$ret_excess))

(US_Stocks %>% filter(month =="2015-01-01"))$size %>% mean()
...
#Test weights
compute_parametric_portfolio_weights(theta,US_Stocks) %>% view()

#Test evaluate
((evaluate_portfolio(compute_parametric_portfolio_weights(theta,US_Stocks)))$Rp %>%mean())#-2*((evaluate_portfolio(compute_parametric_portfolio_weights(theta,US_Stocks)))$Rp %>%var())
((evaluate_portfolio(compute_parametric_portfolio_weights(theta,US_Stocks)))$Rp %>%sd())


-0.025107100793


tmp2<-(compute_parametric_portfolio_weights(theta,US_Stocks)%>% filter(month == "2000-01-01") %>% select("ret_excess", "weight_tilt"))$weight_tilt
