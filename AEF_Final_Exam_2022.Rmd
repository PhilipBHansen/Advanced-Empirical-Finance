...
k_months_ahead_alphas %>%
  knitr::kable(booktabs = T, 
               digits = 4, 
               col.names = c("Portfolio number", "Alpha 1 month ahead", "Alpha 3 months ahead", "Alpha 6 months ahead", "Alpha 12 months ahead"), 
...
Most academic studies ignore real world costs, this could prove to be an issue for the momentum portfolio. We have seen that the momentum strategy has quite a high turnover and this might lead to momentum becoming too expensive to trade compared to its return premium.
It is very likely that the turnover of a size sort portfolio would be significantly smaller than that of a momentum portfolio. A company that is large will most 
likely remain so in the foreseeable future. Take an example as Apple, and imagine its stock price dropping 10% this would imply severe negative momentum but i would for a decent amount of time remain a huge company. The same goes for really small companies, imagine your local bakery doing an IPO and doubling its market cap in a year, that is enormous positive momentum but that does not make it a Fortune 500 or anything close to it.

The assumption of perfectly divisible assets might also have an effect on cost in the real world. However this problem dissapears as the value of the portfolio becomes sufficiently large. Asset liquidity might also be an issue as some assets simply cant be traded at the pace a momentum strategy requires, at least not without trading at a discount which most likely would leave the strategy worse off.

To capture the momentum premium but lower turnover we suggest a strategy that chooses assets the same way as the *standard long-short* strategy. Instead of dropping the position as soon as its momentum moves out of the top or bottom 10%, we hold it until it hast moved out of the top or bottom 20%.
...
We see that our suggested strategy has a better sharpe ratio, so a better return proportional to the amount of risk taken.The positive excess return of 0.0047 compared to 0.0075 suggests, that we are able to catch a good part of the momentum premium but with a turnover that is about 25% smaller while also taking on less risk. We therefore conclude that the strategy works decently for the assigned purpose.
...
\begin{align*}
    V\left(r_i\right) = E\left[\left(r_{i,t} - E\left[r_{i,t}\right]\right)^2\right] = \frac{1}{N}\sum_{t=1}^N\left(r_{i,t}^2\right) - \left(\hat{\mu}_i^{FF}\right)^2
\end{align*}
\begin{align*}
...
portfolio_weights %>%
  knitr::kable(booktabs = T, 
               digits = 4, 
               caption = "Mean-variance efficient portfolio weights from using the Fama French 3-factor model-implied $\\hat{\\boldsymbol{\\mu}}^{FF}$ and $\\hat{\\boldsymbol{\\Sigma}}^{FF}$") %>%
  kable_paper("hover", full_width = T) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
The parameter vector $\theta$ is a vector coefficient that we estimate. Here we do not estimate one weight for each stock for the different points in time, and we would instead estimate weights as a single function for the chosen characteristics that will apply to all stocks over time, which is called a portfolio policy.

We parameterize the optimal portfolio weights as a function the characteristics for the stock's, with the simple linear specifications for the weights:

$$\omega_{i,t}=f(x_{i,t};\theta)=\bar\omega_{i,t}+\frac{1}{N_t}\theta^{\intercal}\hat x_{i,t} $$

To estimate $\theta$ we maximize the linear policy case  function:
...
Furthermore, based on this, if we have two stocks that are close to each other in characteristics related to the expected return and risk, with different sample returns, they should even have the same weight in the portfolio.

It is relatively easy to optimize a portfolio composed of many stocks. However, the burden of this type of optimization does not come from the number of assets but more from the number of characteristics. Also, only optimize the entire portfolio based on the parameters we choose, which will reduce the risk of in-sample and overfitting. This is because the coefficients will only deviate from zero if there is consistently a combination of risk and return across assets. This will also affect the optimization because it tends not to take extreme values.   

<<<<<<< HEAD
To estimate $\hat{\theta}$ we program a series of functions to determine the parametric portfolio given $x_{i,t}$ and $\theta$. We then implement a function to determine the portfolio's gross return, calculate its certainty equivalent and use the *optim* function in R to do numerical optimization of the certainty equivalent by adjusting $\hat{\theta}$.
=======
To estimate $\hat{\theta}$ we program a series of functions to determine the parametric portfolio given $x_{i,t}$ and $\theta$. We then implement a function to determine the gross return og said portfolio, calculate its certainty equivalent and use the *optim* function in R to do numerical optimization of the certainty equivalent by adjusting $\hat{\theta}$.We define our initial $\hat{\theta}$ as $[\begin{matrix} 1 & 1 & 1 \end{matrix}]'$
>>>>>>> f77a776d5cc4531342e03fa70554b0ad4ee7b09b
...
theta_hat_table %>%
  knitr::kable(booktabs = T, 
               ...
               caption = "Optimal $\\hat{\\theta}$ estimated on data up until december 31st 2015") %>%
  kable_paper("hover", full_width = F) %>%
  kable_styling(latex_options = "HOLD_position")
...
rownames(w_pp_table) <- "Optimal weights"
...
w_pp_table %>%
  knitr::kable(booktabs = T, 
               ...
               caption = "Optimal parametric portfolio as of december 2015 given $\\hat{\\theta}$ estimated on data up until december 31st 2015") %>%
  kable_paper("hover", full_width = T) %>%
  kable_styling(latex_options = "HOLD_position")
...
The different strategies show us that Fama French has the highest total return, but its sharp ratio is only the second highest Sharp Ratio. The highest sharp ratio comes from the Naïve portfolio, which has the second highest total return. This shows us that the best portfolio is the Naïve portfolio compared to the risk we have taken. 

With back-testing strategies, a problem with the estimation can come when we use a very complex model on a dataset that is too small. As our estimation will try to fit more of the data, this can lead to overfitting. A shortfall for this implementation is that we create a portfolio that should hold for 5 years, without rebalancing. Which is a hard task, as we try to predict that far in the future based on the historical data.

The Parametric portfolio strategi in a high-dimensional asset universe will be affected if it needs more characteristics as parameters, as that will make it heavier to optimize. So we expected it to perform better if we add more characteristics, but it will also make the calculation heavier and therefore take more time. The Naïve portfolio will with a certain number of assets eliminate the diversifiable risk, but still, leave the systematic risk. An efficient portfolio will in this kind of universe have can have problems if the time horizen is to small, as it need a long time horizon to calculate the covariance matrix. Farma French will perform well if it is given a time horizon that is long enough, as the investor has to ride out underperformance and volatility that could occur in a short term.  

From the chosen portfolio weights we have aqquired from all 4 methods our assesment is that the efficient will have the largest turnover and therefore the largest transaction costs. Fama French and parametric will have about the same turnover and the naive portfolio will have by far the lowest turnover as it only has to be adjusted marginally to compensat for returns.
