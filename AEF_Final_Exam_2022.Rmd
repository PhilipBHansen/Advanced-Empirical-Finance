...
The whole report is made in close collaboration with each other where we have all helped with each problem. The main author of each section can be seen as follows:  
...
In order to get a sense of the data, we create the following summary statistics, where we calculate the mean, standard deviation and quartiles for both the excess return and the market capitalization for each month and then we average these results over all months. We considered deflating the marketcap with the consumer price index as inflation also naturally drives the market cap upwards thus mimicking positive momentum. We have chosen not to do so as the assignment said nothing about this, but it could be interesting to investigate further.
...
parameter_estimates <- Sub_US_Stocks %>%
  group_by(permno) %>%
...
parameter_estimates %>%
  knitr::kable(booktabs = T, digits = 4, col.names = c("Permno", "Estimate", "sd", "Estimate", "sd", "Estimate", "sd", "Estimate", "sd"), caption = "Fama French 3-factor estimates for the 8 stocks", align = "c") %>%
...
```{r, echo=FALSE}
mu <- compute_fama_french_mu(parameter_estimates, Sub_US_Stocks)
Sigma <- compute_fama_french_sigma(parameter_estimates, Sub_US_Stocks)
...
theta_hat_table %>%
  knitr::kable(booktabs = T, 
               ...
               caption = "Optimal $\\hat{\\boldsymbol{\\theta}}$ values") %>%
  kable_paper("hover", full_width = F) %>%
  kable_styling(latex_options = "HOLD_position")

# Computing optimal weights for last period of the sample
w_pp <- compute_parametric_portfolio_weights(theta_hat, Sub_US_Stocks %>% 
...
# Add the data
strategies_returns[1,] <- c(sum(excess_return * portfolio_weights), sum(excess_return * w_pp), sum(excess_return * efficient), sum(excess_return * naive))
...
Evaluating the strategies’ performances, shows that Fama French generates the highest return over the 5-year period. Despite having the highest return its sharpe ratio is beat by the Naive portfolio. This means the Naive portfolio provides a better return when considering the risk. During back-testing, problems can arise when trying to fit a highly complex model on small dataset as this creates a risk of overfitting. A shortfall of this testing strategy is that we choose a portfolio and stick with it for 5 years. This assumes that stock performance 5 years ahead can be predicted on historical data which seems highly unlikely. 

In a high dimensional asset universe the parametric portfolio will be affected by a change in benchmark, but it will have more data to consistently estimate $\boldsymbol{\theta}$ on. When the universe becomes sufficiently large, the Naive portfolio will have eliminated the diversifiable risk. The efficient portfolio becomes challenging to estimate as the number of assets grows as you will need more observations to determine a nonsingular covariance matrix. Given a long enough time horizon, Fama French will perform well as the investor must ride out underperformance and volatility that could occur in the short term.  

From the portfolio weights we have acquired from all 4 methods, our assessment is that the Efficient portfolio will have the largest turnover and therefore the largest transaction costs. The Fama French and the Parametric portfolios will have about the same turnover and the naive portfolio will by far have the lowest turnover as it only has to be adjusted marginally to compensate for the effect of the returns.
