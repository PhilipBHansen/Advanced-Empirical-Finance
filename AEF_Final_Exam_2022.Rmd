...
We will now derive the model-implied expected gross excess return $\hat{\mu}^{FF}$ and the model-implied variance-covariance matrix $\hat{\Sigma}^{FF}$ . We start by deriving $\hat{\mu}^{FF}$:


**INSERT FROM LATEX WHEN SURE ABOUT CALCULATIONS**
...
# Compute the model-implied expected gross excess return
...
# Compute the model-implied variance covariance matrix
compute_fama_french_sigma <- function(parameter_estimates){
  returns <- compute_fama_french_returns(parameter_estimates, stock_data)
  returns %>% cov() %>% as.matrix()
}

# Function that computes the optimal weights
compute_mean_variance_portfolio_weights <- function(w_prev, mu, Sigma, lambda, gamma = gamma_default){
  N <- ncol(Sigma)
  iota <- rep(1, N)
...
$$\max\limits_{\theta} E_t(u(r_{p,t+1})) =\frac{1}{N}\displaystyle\sum_{i=0}^{T-1} \bigg(\displaystyle\sum_{i=1}^{N_t}\bigg (\bar\omega_{i,t}+\frac{1}{N_t}\theta^{\intercal}\hat x_{i,t} \bigg) r_{i,t+1} \bigg)$$
