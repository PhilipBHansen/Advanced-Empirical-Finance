...
SR <- matrix(c(performance %>% filter(strategy=="Mean-variance with no short-selling") %>% pull("net_return") %>% Sharpe_ratio(),
...
Sharpe_ratio(our_mom_long_short_portfolio$long_short)
...
sigmatilde <- cov(returns_matrix)
mutilde <- colMeans(returns_matrix)
