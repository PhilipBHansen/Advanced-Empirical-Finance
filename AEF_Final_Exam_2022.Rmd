...
sharp_ratio_e4 <- mean_return$ret_excess/sd(mom_portfolios$ret_excess)
```
...
```{r}
 ...
 naive <- 1/ncol(sigmatilde) * rep(1, ncol(sigmatilde))

 efficient <- compute_efficient_weight(sigmatilde, mutilde, w_prev = naive, beta = 0)
 
 excess_return <- Sub_US_Stocks %>% select(permno, ret_excess) %>% group_by(permno) %>% summarise(across(ret_excess,sum))
 
excess_return$ret_excess * w_pp_table
excess_return$ret_excess * portfolio_weights
excess_return$ret_excess * naive
excess_return$ret_excess * efficient
