...
portfolio_weights %>%
  knitr::kable(booktabs = T, 
               digits = 4, 
               caption = "Mean-variance efficient portfolio weights using the Fama French 3-factor model-implied $\\hat{\\boldsymbol{\\mu}}^{FF}$ and $\\hat{\\boldsymbol{\\Sigma}}^{FF}$") %>%
  kable_paper("hover", full_width = T) %>%
  kable_styling(latex_options = "HOLD_position")
```
...
From the chosen portfolio weights we have aquired from all 4 methods our assesment is that the efficient will have the largest turnover and therefore the largest transaction costs. Fama French and parametric will have about the same turnover and the naive portfolio will have by far the lowest turnover as it only has to be adjusted marginally to compensat for returns.
